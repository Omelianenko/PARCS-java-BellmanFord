...
public class Benchmark {
    ...
    public static double getPerformance(HostInfo host) {
        ...
        try {
            ...
            chan.dout.writeByte(Const.DM_PERFORMANCE);
            out.flush();
            if (!Const.NO_TIMEOUTS) host.socket.setSoTimeout(20000);
            try {
                result = chan.din.readDouble();
            } catch (SocketTimeoutException e) {
                System.err.println("getPerfomance: timeout");
            }
        } catch (IOException e) {
            System.err.println("getPerfomance: " + e);
        } finally {
            host.closeSocket();
        }
        return result;
...
//				System.out.println(i);
            ...
            synchronized (r) {
                if (!r.ok)
                    try {
                        if (!Const.NO_TIMEOUTS) r.wait(20000);
                        else r.wait();
            ...
            System.err.println("getBandwidth: " + e);
        } finally {
            host.closeSocket();
        }
        return result;
...
//				System.out.println("total="+total);
            ...
            System.err.println("getBandwidth: timeout");
        } catch (IOException e) {
            System.err.println("getBandwidth: " + e);
        } finally {
            synchronized (this) {
