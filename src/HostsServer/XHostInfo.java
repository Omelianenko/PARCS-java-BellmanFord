...
public class XHostInfo extends HostInfo implements Comparable {
    ...
    public XHostInfo(String name) {
        super(name);
    }

    public XHostInfo(InetAddress ip) {
        super(ip);
    }

    public XHostInfo(String name, InetAddress ip) {
        super(name, ip);
    }

    public static HostInfo getLocalHost() {
        try {
            return new XHostInfo(InetAddress.getLocalHost());
        } catch (UnknownHostException e) {
            return new XHostInfo("localhost");
        }
    }
