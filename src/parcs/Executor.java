...
public class Executor extends Thread {       //поток, в котором запускается АлгМодуль
    ...
    public Executor(String fname) {
        this(fname, loadFile(fname));
    }

    public Executor(String fname, byte[] jarfile) {
        className = getJarMainClass(jarfile);
        fileName = fname;
    }
    ...
    public static String getJarMainClass(byte[] buf) {
        ...
        JarInputStream jar;
        try {
            jar = new JarInputStream(new ByteArrayInputStream(buf));
        } catch (IOException e) {
