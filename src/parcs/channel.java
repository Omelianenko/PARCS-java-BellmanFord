...
/**
 * Канал для связи между двумя точками. Канал создается в родительской точке (на
 * стороне клиента) вызовом из алгоритмического модуля функции
 * point.createChannel(). На стороне сервера объект channel создается демоном и
 * передается в качестве параметра созданному алгоритмическому модулю.
 */
public class channel {
    /**
     * Socket с коротым связан данный канал
     */
    ...
    private channel(InputStream in, OutputStream out) {
        ...
        try {
            ...
            System.err.println("channel: " + e);
        }
    }

    /*
     * Создает канал связи со стороны клиента. Отличается от метода channel(InputStream in,
     * OutputStream out) тем, что создание потоков ObjectInputStream и
     * ObjectOutputStream происходит в обратном порядке. Таким, образом если метод
     * channel(InputStream in, OutputStream out) используется сервером, то клиент
